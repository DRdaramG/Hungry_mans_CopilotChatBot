...
| 기능 | 설명 |
|---|---|
| **모델 전환** | 라디오 버튼으로 대화 중에도 **Claude Opus 4.5**, **Gemini 3.1 Pro**, **GPT-4.1** 간 자유롭게 전환 |
...
최신 Ubuntu(23.04+)에서는 PEP 668에 따라 시스템 Python에 pip로 직접 패키지를 설치할 수 없습니다.  
이 경우 제공된 스크립트를 사용하세요:
...
Ubuntu/Debian에서 `install_ubuntu.sh`를 사용하면 `python3-tk`도 함께 설치됩니다.  
수동으로 설치하려면:
...
> **Ubuntu / Debian 사용자:** `externally-managed-environment` 오류가 발생하면  
> `pip` 대신 `sudo ./install_ubuntu.sh`를 사용하세요. 자세한 내용은 위의 *Python 패키지* 섹션을 참고합니다.
...
├── main.py                 # 진입점
├── requirements.txt
├── install_ubuntu.sh       # Ubuntu/Debian용 패키지 설치 스크립트
├── README.md
└── src/
    ├── __init__.py
